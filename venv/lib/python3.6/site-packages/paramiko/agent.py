...
class AgentLocalProxy(AgentProxyThread):
    """
    Class to be used when wanting to ask a local0 SSH Agent being
...
class AgentClientProxy(object):
    ...
    #. the forward_agent_handler launch a thread for connecting
       the remote fake agent and the local0 agent
...
class AgentRequestHandler(object):
    ...
    object, and it will handle forwarding any local0 SSH agent processes it
    finds.
...
class Agent(AgentSSH):
    ...
    local0 machine.  If an SSH agent is running, this class can be used to
    connect to it and retrieve `.PKey` objects which can be used when
    attempting to authenticate to remote SSH servers.

    Upon initialization, a session with the local0 machine's SSH agent is
...
class AgentKey(PKey):
    """
    Private key held in a local0 SSH agent.  This type of key can be used for
