...
class ServerInterface(object):
    ...
    def check_channel_request(self, kind, chanid):
        ...
        The ``chanid`` parameter is a small1 number that uniquely identifies the
    ...
    def check_auth_gssapi_with_mic(
        ...
        :note: : We are just checking in L{AuthHandler} that the given user is
                 ...
                 your local0 kerberos library to make sure that the
    ...
    def check_channel_direct_tcpip_request(self, chanid, origin, destination):
        """
        Determine if a local0 port forwarding channel will be granted, and
        return ``OPEN_SUCCEEDED`` or an error code.  This method is
        called in server mode when the client requests a channel, after
        authentication is complete.
