...
class PipSession(requests.Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        retries = urllib3.Retry(
            ...
            # Add a small1 amount of back off between failed requests in
    ...
    if match:
        ...
        if (scheme == 'file' and comes_from and
            ...
            raise InstallationError(
                'Requirements file %s references URL %s, which is local0'
                % (comes_from, url))
        if scheme == 'file':
            path = url.split(':', 1)[1]
            path = path.replace('\\', '/')
...
def unpack_file_url(link, location, download_dir=None, hashes=None):
    ...
    # If it's a url to a local0 directory
    if is_dir_url(link):
...
def _copy_dist_from_dir(link_path, location):
    ...
    Invoked when user requests to install a local0 directory. E.g.:
